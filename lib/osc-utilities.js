...
const oscTypeCodes = {
	s : {
		...
		split : (buffer, strict = false) => {
			const split = exports.splitOscString(buffer, strict)
			return {
				value : split.string,
				rest  : split.rest,
			}
	...
	b : {
		representation : 'blob',
		split : (buffer, _strict) => {
			const ref    = exports.splitInteger(buffer)
...
const exportedFunctions = {
	...
	oscTypeCodeToTypeString : (code) => {
		const ref = oscTypeCodes[code]
		return ref !== null ? ref.representation : void 0
	},
...
//TODO: start here.  wtf.
	...
	argToTypeCode : function(arg, strict) {
		var code, value;
